...
| AC | Criterio |
| --- | --- |
| AC1 | Existen las 4 tablas principales con FKs correctas (clientes.userId, compras.clientId). |
| AC2 | Validaciones mínimas de integridad: cantidad > 0, montoTotal > 0, nombreProducto requerido. |
| AC3 | Un usuario puede tener múltiples clientes asociados por userId. |
| AC4 | Aceleradores permiten match por `sku` exacto y/o por `match` parcial. |
