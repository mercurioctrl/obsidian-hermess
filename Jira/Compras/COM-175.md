...
Serie del comprobante (Ej: "A").

`CNUMFAC`

INT

Número de factura (relación con `FACPROT`).
...
Control interno (por defecto 'false').

`LSINIMPINT`

BIT

Sin impuesto interno (por defecto 'true').

`LMODIF`

BIT

Modificado (por defecto 'false').
...
Cálculo automático (por defecto 'false').

`NBASE`

INT

Base de cálculo (por defecto '1').
