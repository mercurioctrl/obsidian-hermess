...
Filtra por ID de cliente (columna CLIENTE_ID)
...
Filtra por categoría (columna CATEGORIA)
...
Filtra por sucursal (columna SUCURSAL)
...
Filtra por fecha de factura (FECHA_FACTURA)
...
Búsqueda libre sobre TITULO o SKU
...
`json` (default) devuelve array JSON. `xlsx` devuelve archivo descargable
