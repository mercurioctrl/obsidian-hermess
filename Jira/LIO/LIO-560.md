...
Cantidad de pedidos liquidados en ese mes. `0` = sin ventas. `-1` = mes aún en procesamiento (solo aplica al mes anterior cuando hoy < día 10).

`mes`

`string`

Nombre del mes en español (ej: `"Enero"`). Viene de `DATENAME(MONTH, ...)`.

`año`

`string`

Año como string (ej: `"2026"`). Viene de `DATENAME(YEAR, ...)`.
...
El vendedor tuvo N ventas liquidadas ese mes.

`0`

El vendedor no tuvo ventas ese mes (mes relleno por la aplicación).

`-1`

El mes aún está en procesamiento (solo aparece en el mes anterior al actual durante los primeros 9 días del mes).
