...
Conexión OK. Se puede continuar con los tests unitarios.

❌ `can_connect_to_the_database` falla

Credenciales incorrectas, servidor inaccesible, o driver `sqlsrv` no instalado en el contenedor.

❌ `can_execute_a_simple_query` falla

La conexión abre pero no puede ejecutar queries. Puede ser un problema de permisos del usuario de DB.

❌ `connected_to_the_correct_driver` falla

El `.env` de testing no está apuntando a `sqlsrv`. Verificar `DB_CONNECTION` en `.env` y en `phpunit.xml`.
