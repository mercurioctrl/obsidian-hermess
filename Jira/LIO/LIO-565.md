...
`app/Service/Inventory/Products/InventoryProductsService.php`

En `extractFilters()`: leer y validar el parámetro `inStock`

`app/Repository/Inventory/Products/ProductsRepository.php`

Agregar JOIN a `stocks`, campo calculado en SELECT, método helper `applyStockFilter()` y uso en `listProducts()` y `countProducts()`

`app/Dto/Inventory/InventoryProductDto.php`

Agregar propiedades `int $stock` y `bool $inStock`
