...
BIGINT UNSIGNED (FK users)

Usuario al que se le asigna un referido

`referral_id`

BIGINT UNSIGNED (FK referrals)

ID del token de referido asignado

`attributed_at`

TIMESTAMP

Fecha en que se atribuyó el referido al usuario

`valid_until`

TIMESTAMP NULL

Fecha hasta la que es válida la atribución (ventana de conversión)
