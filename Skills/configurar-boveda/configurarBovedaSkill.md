...
```bash
curl -sk -H "Authorization: Bearer {OBSIDIAN_TOKEN}" \
...
- El token de Obsidian es personal de cada dev. No hardcodearlo en CLAUDE.md de proyectos compartidos con equipo. En su lugar, usar un placeholder `{OBSIDIAN_TOKEN}` y agregar una nota de que cada dev debe configurar su propio token.
