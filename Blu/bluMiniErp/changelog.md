...
- feat: **mailer `erp` nuevo** = `erp@blustudioinc.com` (box.lio.red:465 SSL), **separado de `payments@`** que queda SOLO para documentos de pago/cobro. `config/mail.php` sumó el mailer `erp` + `erp_from`; vars `MAIL_ERP_*` en `docker-compose.yml`; clave real en `mini-saas/.env` (gitignored). Ver [[Stack e Infraestructura#Mail SMTP]].
