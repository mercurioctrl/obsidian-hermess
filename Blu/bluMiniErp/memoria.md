...
Ultima sincronizacion: 2026-09-16 (**[[Modulo Reservas Reuniones]] — link de videollamada fijo**: `booking_configs.enlace_videollamada` → se inyecta en el `.ics` como `X-GOOGLE-CONFERENCE` para que Google Calendar muestre el botón "Unirse" nativo — los eventos importados por `.ics` no dejaban agregar Meet a mano; mig 0116, PR #64. Vacío = funciona igual que antes). Antes: 2026-09-08 (**fix PDF de activaciones**: portrait paginado con Browsershot/Chromium en vez de html2pdf landscape — repite cabecera de tabla y no parte filas, PR #62, ver [[Errores Comunes]]. Y **[[Modulo Contabilidad]] multi-empresa**: dos razones sociales BLU + DIGITO BINARIO con `empresa_id` en gastos/comprobantes, mig 0114/0115, solo etiquetado, filtro BLU/DIGITO/Todas + reasignar inline, arranca en BLU, PR #63. Ambos en PRs abiertos, ramas desde commit ya en main. También ops: restore de backup local). Antes: 2026-09-05 ([[Modulo Requerimientos]] tarjetas ricas — comentarios/adjuntos(enlace con fetch de título+anti-SSRF)/checklist/fecha límite, mig 0112, PR #58 mergeado). También ops: restore limpio de backup local + deploy, con gotcha nuevo en [[Errores Comunes]] (import a migración vieja → drop total + reimport). Antes: 2026-09-03 portal del cliente: [[Modulo Novedades]] blog público, mig 0110 — y [[Modulo Requerimientos]] tablero Trello en el mismo portal/token, mig 0111, PR #56, con notificación a admins y convertir-en-tarea; ambos seguimiento, no tocan finanzas. Antes: 2026-08-29 [[Modulo Flota GSM]] (mig 0109); 2026-08-23 Dashboard Rentabilidad por Cliente, nombres de cliente clickeables, simulador de impuestos, compras incompletas en Contabilidad; 2026-07-13 auditoría técnica + PR #10)
...
- **Multi-empresa / dos razones sociales (2026-09-08):** el negocio tiene **BLU INC S.R.L** (principal, factura por AFIP) y **DIGITO BINARIO SRL** — **contabilidades separadas**. Cada gasto (compra) y comprobante (venta) cuelga de una `empresa_id` (mig 0114/0115). **Solo etiquetado**: AFIP emite bajo BLU; a DIGITO se le asignan facturas de compra a mano. En `/contabilidad` el filtro **arranca en BLU** (no "Todas") para que el Libro IVA baje por empresa. Ver [[Modulo Contabilidad#Multi-empresa — dos razones sociales (2026-09-08)]].
...
- **Link de videollamada fijo → botón Meet nativo (migración 0116, PR #64, 2026-09-16):** un evento importado a Google Calendar **desde un `.ics`** no muestra el botón "Añadir Meet" (limitación de Google, ni siendo `ORGANIZER`). Fix estilo Calendly free: `booking_configs.enlace_videollamada` (link fijo Meet/Zoom/Teams, self-service en Mi Disponibilidad). `IcsBuilder::invite` gana prop `conference` → emite `X-GOOGLE-CONFERENCE` (botón "Unirse" nativo) + `LOCATION` (si no hay ubicación física) + `DESCRIPTION` + botón en el email. **Vacío = igual que antes.** Meet único por reunión = OAuth Google Calendar API (no implementado). Ver [[Modulo Reservas Reuniones]].
