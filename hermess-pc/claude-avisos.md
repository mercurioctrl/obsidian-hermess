...
5. **Cancelar con Ctrl+C no dispara ningún hook** —- ni `Stop` ni `StopFailure` —- así que las caritas se quedaban rotando para siempre en una sesión que ya no hacía nada. Se resuelve con la misma `Notification` de sesión ociosa (`Claude is waiting for your input`, que Claude Code manda a los ~60 s de idle): no notifica ni suena, pero es **prueba de que la sesión dejó de trabajar**, así que si el estado era `trabajando`/`espera`/`compactando`/transitorio lo corrige a 🔵 y resetea los contadores. Si el estado ya era 🟢 o 🔴 **no lo toca**: esos tienen que seguir esperando a que mire la pestaña. Demora hasta un minuto en corregirse, que es cuando llega la notificación.
   Los mensajes que llegan quedan en `$XDG_RUNTIME_DIR/claude-estado/notif.log`, y los eventos de hook en `eventos.log` —- útil para ver qué dispara (y qué no) cada acción.
