...
---
*Última sincronización: 2026-05-20 — Feature **Reimport de planilla Laset via UI** completado: botón "Examinar" en Sync Laset, validación estricta de headers (orden+nombre), dedup por SHA256 de las 67 columnas, filas nuevas entran como `match_status='NEW'` sin matching ERP. SQL `2026_05_20_001` aplicado + backfill ejecutado (3007 hashes). Infra Docker: `upload_max_filesize=100M` permanente vía `docker-compose.yml`, Python `openpyxl` agregado al `Dockerfile`. Ver [[feature-laset-import#14. Reimport de planilla via UI (2026-05-20)]] y [[changelog]] para detalle.*
