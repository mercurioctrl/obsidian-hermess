...
## Cuenta corriente de proveedores (comp=11)

Destino `NEW_BYTES.dbo.MS_MOV_CTACTE_PROVEEDORES` (análogo de clientes `MC_CCORRIENTES_MOVIMIENTOS`). Reglas:
- **Clave = `FP_Proveedores.CCODPRO`** (código legacy 6 dígitos), NO el `Id_Proveedor` interno. La pantalla del proveedor y todos los movimientos legacy keyean por CCODPRO. Asus Id_Proveedor 16679 → CCODPRO `002605`.
- `IMPORTE_USD` = magnitud positiva; el signo lo da `TR_CODIGO` (factura/deuda→38, pago→40, NC/crédito→30, débito→32). Tablas TR en `NEW_BYTES.dbo.GL_TRANSACCIONES`.
