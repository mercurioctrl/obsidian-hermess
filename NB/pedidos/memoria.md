...
Rama `feature/refacturar-otra-empresa` (back + front, **sin commitear**). Botón derecho sobre un pedido liquidado y facturado → acredita, corrige la liquidación **sin borrarla** (UPDATE, no DELETE, para no romper IDs de un pedido ya entregado) y refactura por otro emisor. La percepción la decide **`FP_Empresas.percepciones` de la empresa DESTINO**, ignorando `clientes.excluirPercepcion` (ese flag está en 1 en los 96.432 clientes de DIGITO y refleja la empresa vieja); solo la sucursal 0010 anula siempre. Permiso `rebillCompany`, auditoría en `NB_WEB.dbo.refacturacion_empresa` (guarda el `voucherCompanyCode` original: el proceso lo cambia para emitir y lo revierte en `finally`). Orden crítico: `getVoucherType()` va **después** de cambiar la empresa o la factura sale por la vieja. Ver [[feature-refacturar-otra-empresa]].

## (2026-09-16) Cuenta corriente — nunca editar, asentar
`VoucherService::impactCurrentAccount` **inserta** movimientos, nunca hace UPDATE, y ese es el patrón a seguir: la deuda original puede estar ya cancelada y pisar ese asiento escondería la deuda nueva dentro de uno saldado. TR_CODIGO: **24** Remitos-Ventas (liquidación, suma deuda), **42** Cobro Efectivo Caja (resta), **32** Débitos Varios (suma), **16** Cta Cte-Cobrar al Cliente (resta). El saldo suma TODOS los movimientos con signo por código (`LiquidateRepository::sellerCredit`); el importe se guarda siempre positivo. **Gotcha:** sobre un remito conviven el cargo 24 y los pagos 42 — buscar "el último movimiento" agarra un pago, filtrar por `TR_CODIGO = 24`. `ID_CCMOVIMIENTO` no es IDENTITY (`MAX + 1`). Ver [[contexto#Cuenta corriente y comprobantes]].

## (2026-09-16) Importes: TOTALREMITO es `real`, usar UPDATE relativo
En `MS_REMITO_CABECERA`, `TOTALREMITO` y `TOTALSINIVA` son **`real`** (4 bytes, ~7 dígitos) e `IMPPERCEP` es `float` (8 bytes). Escribir un total recalculado y redondeado a 2 decimales mete deriva **aunque el valor no haya cambiado**. Regla: `col = col + :delta`, y redondear solo para mostrar.

## (2026-09-16) API de comprobantes va por https
`API_VOUCHER_URL` = **`https://ms-comprobantes.lio.red/v2`**. La variante http da 404 en todas las rutas (`/auth/login`, `/v1/...`, `/v2/...`). El `.env` local tenía un bloque comentado con la URL https correcta pero **password caduca**: sirve la URL nueva con las credenciales que ya estaban activas.

## (2026-09-16) Levantar el front de pedidos
El proceso PM2 es **`WebExpedition`** (puerto 3702, proxy Apache `pedidos.local`), y arranca con `pm2 start ecosystem.config.js` desde `pedidos-web-app-v1/app/`. **Ojo:** en la misma máquina corren otras apps que se confunden fácil — `WebAppLO` es `/var/www/lo/sitio-web-app-v3` y el puerto 3002 es `cobros-web-app-v1`. Verificar con `pm2 describe <name> | grep "exec cwd"` antes de reiniciar nada.
