...
---
*Última sincronización: 2026-09-10 — feat: **anular ingreso de proveedor** (`DELETE /v1/providerOrderInbound/{albaranId}`): revierte stock/costo/estado, deja la orden editable, movimiento inverso en `registro_stock` y auditoría; snapshot de costo en `albprol`; valida no-facturado + ventana 10 días + **seriales no tomados**; front con botón "Anular" + modal de confirmación. Ramas `feature/anular-ingreso-proveedor` (base `Development`/`development`). Requiere DDL manual (`database/sql/anular-ingreso/`) por entorno (solo en 190.210.23.97). Gap: no revierte seriales no-vendidos. Ver [[changelog#2026-09-10|changelog]], [[arquitectura#Anular ingreso (reversa)|arquitectura]] y [[contexto#Anular ingreso de proveedor (2026-09-10)|contexto]].*
*2026-08-09 — feat: check "No tocar costo" en ingresos (3ra modalidad por ítem que saltea el update de `ncosteprom`). API PRs #432→Development / #433→blu-dev-staff; front PRs #299→development / #300→blu-dev-staff. Requiere `ALTER TABLE ... ADD doNotUpdateCost` manual en cada entorno (aplicado solo en 190.210.23.97). Ver [[changelog#2026-08-09|changelog]] y [[contexto#Check "no tocar costo" en ingresos (2026-08-09)|contexto]].*
*2026-07-21 — fix (API, hotfix): `ncosteprom` en ingresos de órdenes en pesos (PSO) ahora se convierte a dólares con `nvaldiv_FISCAL` en vez de `nValDiv` (que en PSO es 1) — antes guardaba el costo promedio en pesos sin convertir (orden 13973: 100 en vez de 0,0667). Rama `hotfix-ncosteprom-cotizacion-fiscal-pso` (base `Development`, `7e12bdc`, pusheada). Corrige el hotfix previo PR #425. Ver [[changelog#2026-07-21|changelog]].*
