...
## Anular ingreso de proveedor (2026-09-10)

Nueva funcionalidad para **deshacer un ingreso (remito)** y dejar la orden nuevamente **editable** (`PedProT.cEstado='p'`). Endpoint `DELETE /v1/providerOrderInbound/{inboundId}` (el `inboundId` es el **albarán** `albprot.nnumalb`, no el nº de orden). Doc completo en `docs/features/anular-ingreso-proveedor.md`. Ver [[arquitectura#Anular ingreso (reversa)|arquitectura]].

- **Reversa** (transacción): resta stock (**se permite negativo** a propósito — se reingresa tras corregir), inserta **movimiento inverso append-only** en `registro_stock` (NO borra el original → queda la traza de la anulación), resta `pedprol.nCanEnt`, limpia flags de costo, restaura `ncosteprom` + `ULTIMO_INGRESO` desde snapshot, `cEstado='p'`, **borra** `albprot`/`albprol`, y audita en `NB_WEB.dbo.provider_inbound_anulacion`.
- **Snapshot de costo**: al ingresar se guardan en `albprol` las columnas `prevCosteProm`, `prevUltimoIngreso`, `costMode` (`noTocar`/`ponderado`/`sobrescribe`) para restaurar el costo exacto. Ingresos previos al feature no tienen snapshot: solo se restauran si fueron "no tocar".
- **Validaciones** (si fallan, no anula): no facturado (`albprot.lfacturado=0`), reciente (`config('inbound.annul_window_days')`, default **10 días**), y **seriales no tomados**.
- **Seriales tomados = bloqueo**: si algún serial del ingreso ya está vendido/egresado/en RMA (`ST_DETALLE_STOCK` con `FECHA_EGRESO`/`ID_VENTA`/`ID_STOCKVENTA`/`ID_RMACLIENTE`/`ID_RMAPROVEEDOR` no nulos), la anulación devuelve **409** nombrando los seriales. Los seriales se ligan por `ID_COMPRA` (nº de ORDEN) + `CREF` (artículo); esta API solo LEE esa tabla, la escribe el sistema de serialización/despacho.
- **Duplicados de ficha 118066 vs 121651**: mismo producto físico (TUF GAMING B650EM-PLUS WI-FI vs WIFI). Los seriales colgaban de `118066`. Para reponer un artículo tras editar la orden alcanza con re-agregarlo por `PATCH /providerOrder/{id}` con `amount` **≥ cantidad de seriales existentes** (reaparecen solos por `ID_COMPRA+cref`); si se manda `amount` menor, la validación de add-item corta con "cantidad serializada (N) no puede ser mayor a la nueva".
- **⚠️ DDL manual** (idempotente, `database/sql/anular-ingreso/`): `ALTER albprol ADD prevCosteProm/prevUltimoIngreso/costMode` + `CREATE NB_WEB.dbo.provider_inbound_anulacion`. Aplicar en cada entorno **antes** de usar la rama (sin las columnas el ingreso mismo falla). Aplicado solo en `190.210.23.97`/`NB_WEB`.

## Deuda técnica / TODOs

- **Reversa de seriales al anular ingreso (PENDIENTE, decisión de negocio 2026-09-10)**: la anulación NO borra/desliga los seriales *no vendidos*. Como se ligan a la ORDEN (no al albarán), en ingresos parciales no hay link serial→albarán directo (habría que acotar por artículo/cantidad/fecha = heurístico); en ingresos completos es directo. Si tras anular no se re-agrega el artículo, esos seriales quedan colgados de la orden. Cuando se retome: borrar (o marcar egreso) los no-vendidos de los artículos del albarán; dejar intactos los vendidos.
