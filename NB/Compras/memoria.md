...
- **Cotización única en el header (front, COM-320, 2026-07-02):** el detalle de Orden/Ingreso muestra una sola "Cotización": pesos (`currencyId==='PSO'`) → `currencyFiscalQuote` (currencyQuote queda en 1), dólares → `currencyQuote`. Computed `currencyQuoteComputed`/`isProviderInPesos`. En `gamma`, pendiente de `development`. Ver [[contexto#Cotización única en el header del detalle (COM-320, 2026-07-02)|contexto]].
