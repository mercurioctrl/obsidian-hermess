...
### Estado 2026-08-08 — Vault MCP (la bóveda como tools nativas de Bily)

Sesión con Claude Opus 5. Síntoma: Bily le dice a Catriel *"no tengo acceso directo a la API de Obsidian desde acá"* al pedirle cargar un pago. Reporte completo: [[Claude/Vault-MCP|Vault MCP]].

- **Causa raíz:** el skill `obsidian-mind` **ya no existe**. Vivía en `~/.openclaw/plugin-skills/obsidian-mind/`, que es un directorio **manejado por los plugins** (no un skill root de usuario) y el update a `2026.7.1-2` del 2026-08-02 lo regeneró, dejándolo huérfano. Además su `SKILL.md` **no tenía frontmatter YAML**, hoy obligatorio. `openclaw skills info obsidian-mind` → *"not found"*.
- **Agravante:** el `AGENTS.md` de Bily lo mandaba explícitamente a ese skill fantasma, sin IP, token ni mención de los wrappers `~/bin/vault-*`. Bily tenía `exec` y 33 tools disponibles y **nunca intentó nada** — en su contexto era verdad que no tenía cómo.
- **Daño colateral:** la bitácora del 2026-08-05 nunca subió, quedó en `~/.openclaw/workspace/memory/pending-obsidian/`. La API estaba OK (`HTTP 200` en 16 ms) todo el tiempo.
- **FIX — servidor MCP `vault`:** `/home/hermess/vault/mcp_server.py` (python3 stdlib, molde de `~/jira/mcp_server.py`). 6 tools: `vault_search`, `vault_get`, `vault_ls`, `vault_append`, `vault_put`, `vault_delete` (guardrail: solo `Bily/` y `Claude/`). Registrado con `openclaw mcp add vault --command python3 --arg /home/hermess/vault/mcp_server.py` + `openclaw mcp reload`. Bily las ve como `vault__vault_*`.
- **`vault_append` es el default de escritura:** el `POST /vault/<path>` del Local REST API hace append nativo. Antes, con solo PUT, había que leer-modificar-reescribir entera cada bitácora — de ahí venían los pisadas de contenido.
- **`AGENTS.md` reescrito:** tabla de las 6 tools + reglas (append > put, `vault_ls` antes de inventar rutas, y **nunca decir "no tengo acceso" sin haber intentado la tool** — si falla, reportar el error textual y dejar lo pendiente en `memory/pending-obsidian/`).
- **Skill roots VÁLIDOS en OpenClaw** (por si algún día hace falta un skill): `<workspace>/skills`, `<workspace>/.agents/skills`, `~/.agents/skills`, `~/.openclaw/skills`, `skills.load.extraDirs`. **Nunca** `~/.openclaw/plugin-skills/` (es de los plugins, se regenera en updates).
- **Lección transversal:** un markdown que el modelo tiene que *acordarse de cargar* es frágil; una tool en su lista siempre está. Es el mismo principio de *intent al LLM, no regex* aplicado al acceso a datos. Jira funcionaba justamente por ser MCP nativo.
- **Verificado:** 6 tools por JSON-RPC directo (incluido el guardrail y el ciclo append→get→delete→404) + smoke test con Bily en sesión aislada → usó `vault__vault_ls` sin ayuda. Backup de config: `openclaw.json.pre-vault-mcp`.
- **Pendiente:** borrar (o no) el `plugin-skills/obsidian-mind/` inerte; registrar el mismo MCP en Claude Code para unificar. La bitácora del 05/08 y el pago de autónomos quedaron para que los cargue Bily como prueba del fix.
