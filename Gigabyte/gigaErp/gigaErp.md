...
**Último commit:** `1db31dc` · **Última sincronización:** 2026-09-15
**Último trabajo (2026-09-15):** **deploy de `Development`** (12 commits) con mejoras en [[modulos/campanas|Campañas]] (edición desde engranaje, calendario propio, cliente obligatorio, estado por-cliente), [[modulos/tareas|Tareas]] (editor enriquecido en comentarios, fix sesión de calendario) y [[modulos/solicitudes|Solicitudes]] (estados aprobada/rechazada/pendiente + backfill de autor). Se **destrabó** el bloque de migraciones **0115–0121** (nunca se habían copiado al container; la DB estaba en 0114) → ahora al día, **sin pérdida de datos**. Ver [[changelog#2026-09-15 — Deploy Development: mejoras Campañas/Tareas/Solicitudes + se destraban migs 0115–0121|changelog]] y [[contexto#Estado de migraciones en dev (act. 2026-09-15 — RESUELTO)|contexto]].
**Trabajo previo (2026-09-08):** **[[modulos/dashboard|Dashboard]] reducido** (solo Tareas + Calendario) + tabla nueva de **POEs subidas** (Reseller · Acción/envío · POEs · Última subida) con **descarga** desde el chip (directo si es 1, modal si son varios). De paso se recuperó un **crash-loop del backend** (config cache en 0 bytes; ver [[troubleshooting#16. Backend en crash-loop por config cache en 0 bytes|troubleshooting #16]]) y se endureció `docker-entrypoint.sh`. PRs #44 (mergeada) / #45 (abierta). Ver [[modulos/dashboard]] y [[changelog#2026-09-08 — Dashboard reducido + descarga de POEs (+ recuperación de crash-loop)|changelog]].
